...
public class Queen extends Piece{
    ...
    //stores the side that the Queen is on (black/white)
    private String side;
    //stores the x position of the Queen
    private int xPos;
    //stores the y position of the Queen
    private int yPos;
    //stores the image of the Queen
    private BufferedImage image;


    //constructor of all Queen Objects must take in its x position, y position and weather its on the "black" or "white" side
    public Queen(int xPos, int yPos, String side){
        //updates all of the instance variables
        this.xPos = xPos;
        this.yPos = yPos;
        this.side = side;

        //used to hold the image of the queen
        try {
            //checks to see if the side parameter of the constructor was "white"
            if(side.equals("white")){
                //means the image instance variable will hold the white Queen image from the images folder
                this.image = ImageIO.read(new File("Images/WhiteQueen.png"));
            }
            //means that the side that was chosen was "black"
            else{
                //means the image instance variable will hold the black Queen image from the images folder
                this.image = ImageIO.read(new File("Images/BlackQueen.png"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        for(int i = 1; i < 8; i++){
            ...
            if(this.yPos+i <= 7){
                ...
                if(pieces[yPos+i][xPos] != null){
                    //break is needed so that the queen does not phase through materials
                    break;
                }
            }
            else{
    ...
    private ArrayList<Integer> newMove(Piece[][] pieces, ArrayList<Integer> moves, int y, int x, boolean kingDanger, int kingXPos, int kingYPos, Piece[][] p){
        ...
        if(pieces[yPos+y][xPos+x] == null || !pieces[yPos+y][xPos+x].getSide().equals(this.side)){
            ...
            if(kingDanger){
                //removes the old spot of the queen from the board
                ...
                //updates the the queen postion
                pieces[yPos+y][xPos+x] = new Queen(xPos+x, yPos+y, side);
                ...
                //restores the piece that the queen took
                pieces[yPos+y][xPos+x] = p[yPos+y][xPos+x];
                //restores the Queen to its original position
                pieces[yPos][xPos] = new Queen(xPos, yPos, side);
            }
